...
{
    /// <summary>
    /// Base class to derive to write you own custom Elemental effect that can be added to a StatsSystem. There is
    /// a default implementation called ElementalEffect that can be used to make Physical/Fire/Electrical/Cold damage
    /// across time
    ///
    /// Derived class *must* implement the Equals function so we can check if 2 effects are the same (e.g. the default
    ...
    /// <summary>
    /// Default implementation of the BaseElementalEffect. The constructor allow to specify what type of damage is done,
    /// how much is done and the speed (time) between each damage (by default 1, every second).
    /// </summary>
